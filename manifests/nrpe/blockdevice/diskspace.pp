...
#
# This will take a drive reference as the name, and use it to create a diskspace check. It will also look up the size of the
# drive to determine the warning/critical thresholds as follows:
# Disk Size  < 100GB:
#   Warning  = 20%
#   Critical = 10%
# Disk Size  > 100GB:
...
define nagios::nrpe::blockdevice::diskspace {
  ...
  if $name != 'xvdd' and $name != 'sr0' {
    ...
    # This has to use a getvar method to return a fact containing another variable in the name.
